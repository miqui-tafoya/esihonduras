...
class EntryinfopublicaModel extends Model {
    ...
    private function getPosts($query) {
        ...
        $sql = "SELECT e.tb_entradas_id, e.entradas_timestamp, e.categoria, e.entradas_titulo, e.publicado, c.nombre_categoria FROM tb_entradas e
        JOIN tb_categorias c ON c.tb_categorias_id=e.categoria 
        WHERE e.tipo = 'publica'
    ...
    public function setIds() {
        $res_pp = 10;
        $sql = "SELECT e.tb_entradas_id FROM tb_entradas AS e WHERE e.tipo = 'publica'";
    ...
    public function validateId($id) {
        if (empty($id)) {
            header('Location:' . URL_BASE . 'entryinfopublica/1');
            exit;
        }
