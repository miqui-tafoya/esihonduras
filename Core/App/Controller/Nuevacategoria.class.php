...
class Nuevacategoria extends Render {
    ...
    public function validaTema($input) {
        ...
        $slug = $model->getSlugs();
        if (in_array(str_replace(' ', '', $input['slug_categoria']), $slug)) {
            $errors = MainCtrl::pushAssoc($errors, 'error_slug', '<i class="fas fa-exclamation-triangle"></i>Esta categoría ya existe');
        }
