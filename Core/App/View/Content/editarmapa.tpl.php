...
$selectRegion = $post['region'] ?? $centro['region'] ?? '';
?>
<div class="dash">
    ...
    <form class="campos-admin" method="post">
        <input type="hidden" name="tb_centros_id" value="<?php echo $post['tb_centros_id'] ?? $centro['tb_centros_id'] ?>">
        ...
        <input class="adm-txt" class="campo" type="text" name="municipio"
            value="<?php echo $post['municipio'] ?? $centro['municipio'] ?>">

        <label for="aldea">Aldea<span class="form-error"><?php echo $error['error_aldea'] ?? '' ?></span></label>
        <input class="adm-txt" class="campo" type="text" name="aldea"
            value="<?php echo $post['aldea'] ?? $centro['aldea'] ?>">

        <label for="nombre">Nombre del Centro<span
                class="form-error"><?php echo $error['error_nombre'] ?? '' ?></span></label>
        <input class="adm-txt" class="campo" type="text" name="nombre"
            value="<?php echo $post['nombre'] ?? $centro['nombre'] ?>">
        ...
        <select style="padding:0 0.5rem;text-transform:uppercase;" class="adm-txt" name="region">
            ...
            foreach ($regiones as $key => $value) {
                ...
                if (isset($selectRegion) && $selectRegion == $key) {
                    echo 'selected';
                }
        ...
        <textarea class="adm-field" style="resize: none;" cols="30" rows="2" type="text" name="direccion"
            maxlength="120"><?php echo $post['direccion'] ?? $centro['direccion'] ?></textarea>
        ...
        <textarea class="adm-field" style="resize: none;" cols="30" rows="2" type="text" name="info"
            maxlength="120"><?php echo $post['info'] ?? $centro['info'] ?></textarea>
